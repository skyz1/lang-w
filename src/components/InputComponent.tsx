        ...
        <div className='border-x-2 border-t-2 h-fit relative overflow-y-auto'>
            <div className='h-fit w-full font-mono resize-none z-10 relative'>
                {coloredLines.map((line, i, arr) => <div key={i} className={"flex flex-row leading-[30px] border-b-2"}>
                        <span className='pr-2 w-11 font-bold border-r mr-1 text-right'>{i}</span>
                        <>{line.map(subline => <span className={subline.color}>{subline.text}</span>)}</>
                    </div>
                )}
            </div>
            <textarea className='text-transparent h-full w-full caret-black font-mono resize-none absolute bg-transparent pl-12 leading-[32px] absolute top-0 outline-0 z-50 selection:text-tranparent selection:bg-blue-700/25' onChange={e => props.inputChanged(e.target.value, props.pipeline)} value={props.code} spellCheck={false}></textarea>
        </div>
